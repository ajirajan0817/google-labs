...
test('allows undoing reset and expires after 5s', async () => {
  vi.useFakeTimers()
  render(<App />)
  const incBtn = screen.getByRole('button', { name: /increment count/i })
  const resetBtn = screen.getByRole('button', { name: /reset count/i })

  // Setup: increment and then reset
